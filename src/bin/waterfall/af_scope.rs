...
use crate::theme::{ACCENT, MUTED, OK, SURFACE, TRACE, TRACE_GLOW, WARN};
...
fn hint_detail(h: AudioLevelHint) -> &'static str {
    match h {
        AudioLevelHint::Idle => "Waiting for audio",
        AudioLevelHint::TooQuiet => "Raise RF gain — trace should reach ±half scale",
        AudioLevelHint::SweetSpot => "Good headroom — AGC not pinned on noise",
        AudioLevelHint::TooHot => "Lower RF gain — trace clipping, AGC riding noise",
    }
}
    ...
    Frame::new()
        ...
        .show(ui, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.label(
                    egui::RichText::new("tune RF gain to ±half scale")
                        .small()
                        .color(MUTED),
                );
...
fn status_badge(ui: &mut Ui, hint: AudioLevelHint) {
    ...
    resp.on_hover_text(hint_detail(hint));
}
