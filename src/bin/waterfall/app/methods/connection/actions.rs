...
impl WaterfallApp {
    ...
    pub(crate) fn can_quick_connect(&self) -> bool {
        if let Some(req) = self.connection.recent_hosts.first() {
            Self::can_connect_request(req)
        } else {
            is_local_source(self.connection.form_kind) || !self.connection.form_host.trim().is_empty()
        }
    }

    pub(crate) fn quick_connect_target_label(&self) -> String {
        self.connection.recent_hosts
            .first()
            .map(|r| r.label())
    ...
    pub(crate) fn quick_connect_last(&mut self) {
        if let Some(req) = self.connection.recent_hosts.first().cloned() {
    ...
    pub(crate) fn connect_now(&mut self) {
        ...
        let sample_rate = match self.connection.form_kind {
            #[cfg(feature = "airspy")]
            SourceKind::Airspy => self.connection.form_sample_rate,
            #[cfg(feature = "rtlsdr")]
            SourceKind::RtlSdr => self.connection.form_sample_rate,
            #[cfg(feature = "qmx")]
            SourceKind::Qmx => 0,
            _ => 0,
        };
        let mut kiwi = self.connection.form_kiwi.clone();
        kiwi.rf_agc_on = self.radio.agc_rf_on;
        let req = ConnectRequest {
            kind: self.connection.form_kind,
            host: self.connection.form_host.trim().to_string(),
            port: self.connection.form_port,
        ...
        self.connection.last_airspy_rf = self.connection.form_airspy.clone();
        self.connection.last_rtlsdr_rf = self.connection.form_rtlsdr.clone();
        self.connection.last_qmx_rf = self.connection.form_qmx.clone();
    ...
    pub(crate) fn remember_host(&mut self, req: &ConnectRequest) {
        self.connection.recent_hosts.retain(|r| r != req);
        self.connection.recent_hosts.insert(0, req.clone());
        self.connection.recent_hosts.truncate(8);
    }
