...
impl WaterfallApp {
    ...
    pub(crate) fn connection_form_section(&mut self, ui: &mut egui::Ui) {
        popup_section(ui, "Connect", None, |ui| {
            ...
            let selected = source_kind_index(self.connection.form_kind);
            if let Some(i) = segment_choice(ui, "source_kind", selected, &labels) {
                self.connection.form_kind = source_kind_from_index(i);
            }
            ...
            if self.connection.form_kind == SourceKind::Kiwi {
                ui.horizontal(|ui| {
                    ...
                    ui.add(
                        egui::TextEdit::singleline(&mut self.connection.form_host)
                            .hint_text("kiwi.example.com")
                            .desired_width(ui.available_width() - 72.0),
                    );
                    ui.label(egui::RichText::new("Port").small().color(MUTED));
                    ui.add(egui::DragValue::new(&mut self.connection.form_port).range(1..=65535));
                });
            }
            ...
            let can_connect = is_local_source(self.connection.form_kind) || !self.connection.form_host.trim().is_empty();
            ui.horizontal(|ui| {
