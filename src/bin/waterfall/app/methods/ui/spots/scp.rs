...
impl WaterfallApp {
    ...
    pub(crate) fn scp_body(&mut self, ui: &mut egui::Ui) {
        let scp = &self.engine_ui.stats.scp;
        let downloading = self.skimmer_ui.scp_download_rx.is_some();
        popup_section(
            ui,
            "MASTER.SCP",
            Some("Callsign database — fewer false decodes when loaded"),
            |ui| {
                if scp.loaded {
                    ...
                    if let Some(path) = &scp.path {
                        ...
                        let shown = truncate_middle(path, 34);
                        let truncated = shown != *path;
                        let resp = ui.label(
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.button("Reload").clicked() {
                        ...
                        self.skimmer_ui.scp_reload_deadline =
