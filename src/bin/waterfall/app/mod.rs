...
/// Whether the page is served over TLS, so sockets must be `wss://`.
///
/// The browser blocks a `ws://` connection from an https page as mixed
/// content, and reports it as a generic connection failure — so the UI has to
/// warn about it up front or the user has no way to tell that apart from a
/// receiver being down.
...
impl WaterfallApp {
    ...
    /// Browser build: the real UI over a real engine.
    ///
    /// The engine runs in-process rather than on a thread — see
    /// [`EngineHandle::spawn_in_process`] — and is stepped once per frame. The
    /// only source a tab can reach is a KiwiSDR, over the browser's WebSocket
    /// API; there is no synthetic fallback, so the waterfall stays empty until
    /// the user connects to a receiver.
    #[cfg(all(target_arch = "wasm32", not(feature = "gui-core")))]
    pub fn new_for_web() -> Self {
        let mut app = Self::build(None, EngineHandle::spawn_in_process());
        app.apply_settings(&AppSettings::default());
        app.connection.form.kind = SourceKind::Kiwi;
        app.connection.form.port = 8073;
        // Nothing has connected yet, so open the drawer the user needs first.
