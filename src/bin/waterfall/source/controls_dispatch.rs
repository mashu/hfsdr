...
pub fn kiwi_set_rf_agc(source: &mut DeviceSource, on: bool) {
    if let DeviceSource::Kiwi(s) = source {
        log::warn_if_err("Kiwi RF AGC", KiwiControls::set_agc(s, on));
    }
}

pub fn kiwi_set_man_gain(source: &mut DeviceSource, gain: u8) {
    if let DeviceSource::Kiwi(s) = source {
        log::warn_if_err("Kiwi manual gain", KiwiControls::set_man_gain(s, gain));
    }
}

pub fn kiwi_set_rf_attn_db(source: &mut DeviceSource, db: f32) {
    if let DeviceSource::Kiwi(s) = source {
        log::warn_if_err("Kiwi RF attenuation", KiwiControls::set_rf_attn_db(s, db));
    }
}
...
pub fn airspy_set_hf_att(source: &mut DeviceSource, step: u8) {
    if let DeviceSource::Airspy(s) = source {
        log::warn_if_err("Airspy HF att", AirspyControls::set_hf_att(s, step));
    }
}
...
pub fn airspy_set_hf_lna(source: &mut DeviceSource, on: bool) {
    if let DeviceSource::Airspy(s) = source {
        log::warn_if_err("Airspy HF LNA", AirspyControls::set_hf_lna(s, on));
    }
}
...
pub fn airspy_set_hf_agc_threshold(source: &mut DeviceSource, high: bool) {
    if let DeviceSource::Airspy(s) = source {
        log::warn_if_err(
            ...
            AirspyControls::set_hf_agc_threshold(s, high),
        );
    }
}
...
pub fn airspy_set_frontend_options(source: &mut DeviceSource, flags: u32) {
    if let DeviceSource::Airspy(s) = source {
        log::warn_if_err(
            ...
            AirspyControls::set_frontend_options(s, flags),
        );
    }
}
...
pub fn airspy_set_bias_tee(source: &mut DeviceSource, on: bool) {
    if let DeviceSource::Airspy(s) = source {
        log::warn_if_err("Airspy bias tee", AirspyControls::set_bias_tee(s, on));
    }
}
...
pub fn rtlsdr_set_rtl_agc(source: &mut DeviceSource, on: bool) {
    if let DeviceSource::RtlSdr(s) = source {
        log::warn_if_err("RTL-SDR AGC", RtlSdrControls::set_agc(s, on));
    }
}
...
pub fn rtlsdr_set_manual_gain(source: &mut DeviceSource, manual: bool) {
    if let DeviceSource::RtlSdr(s) = source {
        log::warn_if_err("RTL-SDR manual gain mode", RtlSdrControls::set_tuner_gain_mode(s, manual));
    }
}
...
pub fn rtlsdr_set_tuner_gain(source: &mut DeviceSource, gain_db10: i32) {
    if let DeviceSource::RtlSdr(s) = source {
        log::warn_if_err("RTL-SDR tuner gain", RtlSdrControls::set_tuner_gain(s, gain_db10));
    }
}
...
pub fn rtlsdr_set_bias_tee(source: &mut DeviceSource, on: bool) {
    if let DeviceSource::RtlSdr(s) = source {
        log::warn_if_err("RTL-SDR bias tee", RtlSdrControls::set_bias_tee(s, on));
    }
}
...
pub fn rtlsdr_set_ppm(source: &mut DeviceSource, ppm: i32) {
    if let DeviceSource::RtlSdr(s) = source {
        log::warn_if_err("RTL-SDR PPM correction", RtlSdrControls::set_freq_correction(s, ppm));
    }
}
...
pub fn qmx_set_rf_gain_db(source: &mut DeviceSource, db: u8) {
    if let DeviceSource::Qmx(s) = source {
        log::warn_if_err("QMX RF gain", QmxControls::set_rf_gain_db(s, db));
    }
}
...
pub fn soapy_set_gain_db(source: &mut DeviceSource, db: f64) {
    if let DeviceSource::Soapy(s) = source {
        log::warn_if_err("SoapySDR gain", SoapyControls::set_gain_db(s, db));
    }
}
...
pub fn soapy_set_agc(source: &mut DeviceSource, on: bool) {
    if let DeviceSource::Soapy(s) = source {
        log::warn_if_err("SoapySDR AGC", SoapyControls::set_agc(s, on));
    }
}
...
pub fn soapy_set_antenna(source: &mut DeviceSource, name: &str) {
    if let DeviceSource::Soapy(s) = source {
        log::warn_if_err(
            format!("SoapySDR antenna '{name}'"),
            SoapyControls::set_antenna(s, name),
        );
    }
}
